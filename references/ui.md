...
Read this when the patch changes layout, look, or a new surface —
not for a logic patch in an existing screen. `SKILL.md` only decides
when to load this file. Core rules there still apply: don't invent
an existing thing; look it up. Logic-only JSX: [react.md](react.md)
already loaded; kit + `<button>` + `<label>` + `:focus-visible` are
enough — do not pull this file. Strings: [copy.md](copy.md).
Inventing a landing: [uniqueness.md](uniqueness.md)
...
| Images / quotes | [ui/assets.md](ui/assets.md) |
...
| Exactly three equal feature cards, icon-in-rounded-square, Lucide; bento of equal cells on dark | Hierarchy, not a grid to fill. Icon smaller than the type. [cards.md](ui/cards.md) |
...
| Inline `·` (`THIS MAC · 3.9 MB`); `→` as the icon (`Get started →`); dark all-caps mono table (Q / NAME / SIZE) + hairline + acid-green selected row | Match the file. Else comma or a list; kit icon or the verb; kit Table / this product's density. [copy.md](copy.md) [tables.md](ui/tables.md) [assets.md](ui/assets.md) |
...
Name the job before markup: **persuade** (landing), **operate**
(app, settings, table), or **read** (docs). A glass nav or three
feature cards inside a dashboard is the landing stencil on the
wrong job. Famous-app skins (Linear / Raycast / Stripe) are the
next centroid — pair from the subject's world.
...
// noslop — looked up in @heroui/react, then imported
