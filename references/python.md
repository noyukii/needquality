...
No `print(...)` in production modules. No unused imports. Do not leak JS
idioms (`.push`, `.forEach`, `===`). `assert True` is not a test. No
...
CSRF / JWT / webhooks: [trust.md](trust.md) when this patch does HTTP.
