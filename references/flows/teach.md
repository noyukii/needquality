...
Before `RESOURCES.md` is well-populated, focus on high-quality sources that
help the user acquire knowledge. Verify external, current, disputed, or
high-stakes claims against primary sources; stable fundamentals can be used
from existing knowledge when that is proportionate to the lesson.
...
Glossaries, in particular, are an essential reference. Use
[GLOSSARY-FORMAT.md](teach/GLOSSARY-FORMAT.md). Once one is created, adhere to
it in every lesson.
