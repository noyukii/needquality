...
The diagrams carry the weight. Prose is sparse, plain, and uses the glossary terms (from [codebase-design](../codebase-design.md)) without ceremony.
...
Plain English, concise, but the architectural nouns and verbs come straight from [codebase-design](../codebase-design.md). Concision is not an excuse to drift.
...
No hedging, no throat-clearing, no "it's worth noting that…". If a sentence could be a bullet, make it a bullet. If a bullet could be cut, cut it. If a term isn't in the [codebase-design](../codebase-design.md) glossary, reach for one that is before inventing a new one.
