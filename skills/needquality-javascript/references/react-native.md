...
`expo`, after [javascript.md](javascript.md) (and
[typescript.md](typescript.md) for `.ts` / `.tsx`). **Skip
[react.md](react.md)** — that file is web (`div`, `next/image`,
`'use client'`, `:focus-visible`). Layout or a new look: the
`needquality-ui` skill. HTTP / auth / money: the `needquality-trust`
skill.
