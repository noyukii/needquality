...
Read this for any `.js` / `.ts` / `.jsx` / `.tsx` / `.vue` / `.svelte`
patch. The contract in `SKILL.md` still applies. `SKILL.md` names the
framework and TypeScript references that follow this file.
...
the `needquality-trust` skill when this patch does HTTP or a store.
