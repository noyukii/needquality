...
Read this when touching `.tsx` `.jsx`, after [javascript.md](javascript.md)
(and [typescript.md](typescript.md) for `.tsx`). Layout, look, or a new
surface: the `needquality-ui` skill. Logic-only JSX: kit import +
`<button>` + `<label>` + `:focus-visible` + kit icon for a mark; the
UI skill stays unloaded.
...
App Router files: also [next.md](next.md). A dedicated perf /
design pass loads those skills.
...
an `ignore` flag. Check `res.ok` before `.json()`. A hook that
returns `{ data, refetch }` with no `error` invents success —
loading and fail are not the same state. Don't use a
...
**No new barrel for one import.** Import the file directly unless this
repo already uses a barrel as a deliberate public boundary; match that
boundary and do not widen it.
...
**Marks.** A directional, status, or nav symbol on a control is a
kit icon. Same import as the file; else the kit slot; else inline
SVG (16–24px, `currentColor`). Save / Cancel stay text. Procedure:
the `needquality-ui` skill's assets reference.

```tsx
// slop
<button>Get started →</button>

// needquality — same set as the file
import { ArrowRight } from "lucide-react"
<button>Get started <ArrowRight aria-hidden /></button>
```
