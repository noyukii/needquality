...
name: needquality-go
description: >
  Language rules for Go patches: gofmt, wrapped errors on every path,
  request-scoped contexts and deadlines, bounded goroutines and channels,
  http.Client timeouts, JSON decoding, and table tests in the existing
  package. Use when editing .go files, go.mod, or Go services and CLIs.
---
